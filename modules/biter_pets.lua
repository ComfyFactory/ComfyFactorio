...
function Public.biter_pets_tame_unit(player, unit, forced)
    if storage.biter_pets[player.index] then
        return false
    end
    if not forced then
    ...
    storage.biter_pets[player.index] = { last_command = 0, entity = unit }
...
local function on_player_changed_position(event)
    ...
    if not storage.biter_pets[player.index] then
        return
    end
    if not storage.biter_pets[player.index].entity then
        storage.biter_pets[player.index] = nil
        return
    end
    if not storage.biter_pets[player.index].entity.valid then
        storage.biter_pets[player.index] = nil
        return
    end
    if not player.character then
    ...
    if storage.biter_pets[player.index].last_command + 600 > game.tick then
        return
    end
    storage.biter_pets[player.index].last_command = game.tick
    command_unit(storage.biter_pets[player.index].entity, player)
end
