...
local function get_turret_button(player)
    if Gui.get_mod_gui_top_frame() then
        return Gui.get_button_flow(player).turret_filler_button
    end
    return player.gui.top.turret_filler_button
end
        ...
        if Gui.get_mod_gui_top_frame() then
            b =
                Gui.add_mod_button(
                    ...
                    {
                        type = 'sprite-button',
                        name = 'turret_filler_button',
                        sprite = 'entity/gun-turret',
                        tooltip = { 'modules.turret_filler_tooltip' },
            ...
            b = player.gui.top.turret_filler_button or
                player.gui.top.add({ type = 'sprite-button', name = 'turret_filler_button', sprite = 'entity/gun-turret', tooltip = { 'modules.turret_filler_tooltip' } })
            b.style.minimal_height = 38
            b.style.maximal_height = 38
        end
        b = b or get_turret_button(player)
        if b and b.valid then
            b.visible = turrettable.enabled
        end
    end
end
...
local function turret_gui(player)
    ...
    if turrettable.enabled == false then
        local button = get_turret_button(player)
