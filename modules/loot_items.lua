...
function Public.add(surface, position, chest)
    ...
    local chest_loot =
    {
        ...
        { { name = "bulk-inserter", count = math_random(8, 16) }, weight = 1, d_min = 0.2, d_max = 1 },
        { { name = "stack-inserter", count = math_random(4, 8) }, weight = 1, d_min = 0.4, d_max = 1 },
    ...
    for _, t in pairs(chest_loot) do
        for _ = 1, t.weight, 1 do
            if t.d_min <= distance_to_center and t.d_max >= distance_to_center then
                table.insert(chest_raffle, { name = t[1].name, count = t[1].count, quality = 'normal' })
            end
        end
    end
