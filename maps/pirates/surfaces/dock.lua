...
Public.PurchaseableBoats = {
	[Boats.enum.SLOOP] = {
		...
		position = Utils.snap_coordinates_for_rails({
			x = -23 - Boats[Boats.enum.SLOOP].Data.leftmost_gate_position,
			y = Public.Data.static_boat_bottom - Boats[Boats.enum.SLOOP].Data.height / 2,
		}),
	},
	-- [Boats.enum.CUTTER] = {
...
function Public.place_dock_jetty_and_boats()
	...
	local destination = Common.current_destination()
	if not (boat and boat.surface_name) then
		return
	end
