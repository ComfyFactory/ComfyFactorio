...
function Public.reveal(surface, source_surface, position, brushsize)
	...
	if not source_tile.valid then
		return
	end
	if source_tile.name == 'lab-dark-2' then
		return
	end
	...
	do
		...
		if
			...
			and tile.name ~= 'lab-dark-1'
			and (position.x - tile_position.x) ^ 2 + (position.y - tile_position.y) ^ 2 < brushsize_square
		then
			i = i + 1
					...
					if e.name == 'biter-spawner' or e.name == 'spitter-spawner' then
						...
						if destination.dynamic_data and destination.dynamic_data.disabled_wave_timer then
							destination.dynamic_data.disabled_wave_timer =
								Balance.prevent_waves_from_spawning_in_cave_timer_length
						end
...
function Public.roll_source_surface(destination_data)
	...
	local cave_surface_name = SurfacesCommon.encode_surface_name(
		island_surface_name.crewid,
		island_surface_name.destination_index,
		island_surface_name.type,
		IslandEnum.enum.CAVE_SOURCE
	)
...
local function on_player_changed_position(event)
	...
	if not (destination_data and destination_data.subtype == IslandEnum.enum.CAVE) then
		return
	end
