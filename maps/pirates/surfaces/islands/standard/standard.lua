            ...
            if (not args.iconized_generation) and noises.forest_abs_suppressed(p) < 0.75 and Math.random(1300) == 1 then
                ...
                if forest_noise > 0 then
                    if noises.rock(p) > 0 then
                        ...
                        if Math.random(1, 100) < treedensity * 100 then
                            args.entities[#args.entities + 1] = { name = "tree-06-brown", position = args.p }
                        end
                    end
                elseif forest_noise < -1 then
                    ...
                    if Math.random(1, 100) < treedensity * 100 then
                        args.entities[#args.entities + 1] = { name = "tree-07", position = args.p }
                    end
                end
            end
        end
...
function Public.chunk_structures(args)
    local spec = function(p)
        ...
        return {
            ...
            density_perchunk = 30
                * Math.slopefromto(noises.mood(p), 0.14, -0.1)
                * Math.slopefromto(noises.farness(p), 0.3, 1) ^ 1.5
