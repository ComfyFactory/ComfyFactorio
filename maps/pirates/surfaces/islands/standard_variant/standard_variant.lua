        ...
        if noises.forest_abs_suppressed(p) > 0.7 then
            if (not args.iconized_generation) and noises.forest_abs_suppressed(p) < 1 and Math.random(700) == 1 then -- high amounts of this
                    ...
                    if noises.rock(p) > 0.05 then
                        if Math.random(1, 100) < treedensity * 100 then
                            args.entities[#args.entities + 1] =
                                { name = "tree-08-red", position = args.p, visible_on_overworld = true }
                        end
                ...
                elseif forest_noise < -1.2 then
                    if Math.random(1, 100) < treedensity * 100 then
                        args.entities[#args.entities + 1] =
                            { name = "tree-09", position = args.p, visible_on_overworld = true }
                    end
                else
                    if Math.random(1, 100) < treedensity * 100 then
                        args.entities[#args.entities + 1] = { name = "tree-02-red", position = args.p }
                    end
                end
            end
        end
    ...
    if noises.forest_abs_suppressed(p) < 0.45 then
        if noises.height(p) > 0.12 then
            if noises.rock_abs(p) > 0.25 then
                local rockdensity = 1 / 600 * Math.slopefromto(noises.rock_abs(p), 0.25, 0.6)
                    + 1 / 5 * Math.slopefromto(noises.rock_abs(p), 2.4, 2.6)
...
function Public.chunk_structures(args)
    local spec = function(p)
        ...
        return {
            ...
            density_perchunk = 25
                * Math.slopefromto(noises.mood(p), 0.16, -0.1)
                * Math.slopefromto(noises.farness(p), 0.3, 1) ^ 2
