...
local function radioactive_tick()
	for _, id in pairs(Memory.get_global_memory().crew_active_ids) do
		...
		if destination.subtype == IslandsCommon.enum.RADIOACTIVE then
			...
			local pollution = 4 * (5 * Common.difficulty()^(1.1) * (Common.overworldx()/40)^(16/10) * (Balance.crew_scale())^(0.6)) / 3600 * tickinterval * (1 + 0.001 * (destination.dynamic_data.timer or 0))
