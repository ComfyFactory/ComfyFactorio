...
function Public.process_etaframe_update(player, flow1, bools)
	...
	if bools.cost_bool or bools.atsea_loading_bool or bools.atsea_waiting_bool or bools.atsea_victorious_bool or bools.eta_bool or bools.retreating_bool or bools.leave_anytime_bool then
		...
		elseif bools.atsea_waiting_bool then
			...
			if memory.proceed_are_you_sure_data and memory.proceed_are_you_sure_data[player.index] and memory.proceed_are_you_sure_data[player.index] > game.tick - 60 * 4 then
		...
		if bools.captain_bool and (not bools.retreating_bool) and (bools.leave_anytime_bool or bools.atsea_waiting_bool or bools.atsea_victorious_bool or bools.eta_bool or (bools.cost_bool and (not bools.atsea_loading_bool))) then
...
function Public.update_gui(player)
	...
	if flow1 then
		...
		if not (bools.eta_bool or bools.retreating_bool or bools.quest_bool or bools.silo_bool or bools.atsea_loading_bool or bools.leave_anytime_bool or bools.cost_bool or bools.approaching_dock_bool or bools.leaving_dock_bool or bools.atsea_sailing_bool or bools.atsea_waiting_bool or bools.atsea_victorious_bool) then
			flow1.visible = true
		else
			flow1.visible = false
		end
...
local function on_gui_click(event)
	...
	if event.element.name and event.element.name == 'etaframe_piratebutton' then
		...
		elseif memory.boat.state == Boats.enum_state.ATSEA_WAITING_TO_SAIL then
			if Permissions.player_privilege_level(player) >= Permissions.privilege_levels.CAPTAIN then
				if (not memory.proceed_are_you_sure_data) then memory.proceed_are_you_sure_data = {} end
				if memory.proceed_are_you_sure_data[player.index] and memory.proceed_are_you_sure_data[player.index] > game.tick - 60 * 4 then
