...
local function add_science_logs(player, element)
    ...
    if storage.science_logs_category_potion == nil then
        ...
        for i = 1, 7 do
            table.insert(storage.science_logs_category_potion, get_science_text(food_long_and_short[i].long_name, food_long_and_short[i].short_name))
        end
    end
    ...
    local headersSummary = {
        ...
        [4] = storage.science_logs_category_potion[3],
        [5] = storage.science_logs_category_potion[4],
        [6] = storage.science_logs_category_potion[5],
        [7] = storage.science_logs_category_potion[6],
        [8] = storage.science_logs_category_potion[7]
    }
    ...
    local dropdown_force = t_filter.add { name = 'dropdown-force', type = 'drop-down', items = forces_list, selected_index = storage.dropdown_users_choice_force[player.name] }
    local dropdown_science = t_filter.add { name = 'dropdown-science', type = 'drop-down', items = science_list, selected_index = storage.dropdown_users_choice_science[player.name] }
    ...
    if storage.science_logs_date then
        for i = 1, #storage.science_logs_date, 1 do
            ...
            if dropdown_force.selected_index == 1 or real_force_name:match(dropdown_force.get_item(dropdown_force.selected_index)) then
                ...
                then
                    if
                        dropdown_evofilter.selected_index == 1 or (dropdown_evofilter.selected_index == 2 and (storage.science_logs_evo_jump_difference[i] > 0)) or
                        (dropdown_evofilter.selected_index == 3 and (storage.science_logs_evo_jump_difference[i] >= 10)) or
                        (dropdown_evofilter.selected_index == 4 and (storage.science_logs_evo_jump_difference[i] >= 5)) or
                        (dropdown_evofilter.selected_index == 5 and (storage.science_logs_evo_jump_difference[i] >= 4)) or
                        (dropdown_evofilter.selected_index == 6 and (storage.science_logs_evo_jump_difference[i] >= 3)) or
