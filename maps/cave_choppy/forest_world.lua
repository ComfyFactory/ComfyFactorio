...
local function process_tile(surface, pos, tile, seed)
    ...
    if math_random(1, 100000) == 1 then
        local wrecks = { 'crash-site-spaceship-wreck-big-1', 'crash-site-spaceship-wreck-big-2' }
...
local function init()
    ...
    storage.surface_choppy_elevator = surface.create_entity({ name = 'proxy-container', position = { 1, -4 }, force = game.forces.neutral })
