...
local function map_reset()
    ...
    storage.collapse_x = -96
    storage.collapse_tiles = {}
...
local function on_tick()
    ...
    if tick % 3600 == 60 then
        if storage.reset_railway_troopers then
            if storage.reset_railway_troopers == 1 then
                storage.reset_railway_troopers = 2
                map_reset()
            ...
            if storage.reset_railway_troopers == 2 then
                place_spawn_entities(surface)
                storage.reset_railway_troopers = nil
                return
            end
            return
        end
        ...
        if not wagons[1] then
            ...
            storage.reset_railway_troopers = 1
            return
        else
    ...
    if not storage.collapse_tiles then
        local area = { { storage.collapse_x - 1, negative_map_height }, { storage.collapse_x, map_height + 1 } }
        game.forces.player.chart(surface, area)
        storage.collapse_tiles = surface.find_tiles_filtered({ area = area })
        storage.size_of_collapse_tiles = #storage.collapse_tiles
        storage.collapse_x = storage.collapse_x + 1
        if storage.size_of_collapse_tiles == 0 then
            storage.collapse_tiles = nil
            return
        end
        table.shuffle_table(storage.collapse_tiles)
    end
    for _ = 1, 2, 1 do
        local tile = storage.collapse_tiles[storage.size_of_collapse_tiles]
        if not tile then
            storage.collapse_tiles = nil
            return
        end
        storage.size_of_collapse_tiles = storage.size_of_collapse_tiles - 1
