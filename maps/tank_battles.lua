...
function shrink_arena()
    ...
    if global.current_arena_size < 0 then
        return
    end
...
local function render_arena_chunk(event)
    if not global.noise_seed then
        global.noise_seed = math_random(1, 2097152)
    end
...
local function on_player_joined_game(event)
    ...
    if game.surfaces['nauvis'] then
        ...
        if global.lobby_timer then
            global.lobby_timer = 1200
        end
    end
end
...
local function lobby()
    ...
    if not global.lobby_timer then
        global.lobby_timer = 1200
    end
