...
local function boom_player(player)
    if check_tick(player,'boom_player') then 
        ...
        new_print(player,{'tianfu.boom_player_over'})
    end
    end
    return true
end
...
local function dutu(player)
    if check_tick(player,'dutu') then 
        if player.character.get_item_count('coin')>=1500 and math.random(1,6)==1 then 
...
local function fali(player)
    ...
    game.print({'tianfu.fali_over',player.name})
    end
    return true
end
...
function(player)
    ...
    new_print(player,{'tianfu.xiuxing_over'})
...
local function biter(player)
    ...
    new_print(player,{'tianfu.biter_over'})
...
local function shiyou(player)
    ...
    if check_tick(player,'shiyou') then 
        ...
        new_print(player,{'tianfu.shiyou_over',count*8})
    end
    return true
end
local function tank(player)
    ...
    new_print(player,{'tianfu.tank_over'})
end
...
local function ftlt(player)
    if check_tick(player,'ftlt') then 
        local all= 100
        local t=math.random(30,70)
        player.insert({name = 'iron-plate', count = all-t})
        player.insert({name = 'copper-plate', count =t})
...
local function junhuo(player)
    if check_tick(player,'junhuo') then 
        ...
        new_print(player,{'tianfu.junhuo_over'})
...
local function carxiu(player)
    if check_tick(player,'carxiu') then 
        local position=player.position
        local entities = player.surface.find_entities_filtered {name = car_name,force = player.force, area = {{position.x - 3, position.y - 3}, {position.x + 3, position.y + 3}}}
        local count = 0
        for i = 1, #entities do
          local e = entities[i]
          if e.prototype.max_health ~= e.health then
            e.health= e.health+e.prototype.max_health*0.08
          end
        end
        if #entities ~= 0 then 
...
local function zdfs(player)
    if check_tick(player,'zdfs') then 
        ...
        if #entities ~= 0 then
            ...
            if  player.character.get_item_count('explosive-rocket')>=1 then 
                local  e =  player.surface.create_entity(
                    {
...
local time_skills={
    ...
    ['hd']={name=hd,time=60*60*15},
    ['fali']={name=fali,time=60*30},
    ['carxiu']={name=carxiu,time=60*30},
    ['ftlt']={name=ftlt,time=60*60*5}
...
local trigger_skills={
    ...
    ['xueshu']={name=xueshu},
    ['morefali']={name=morefali,time=60*60*30},
    ['willdie']={name=willdie,time=60*60*10},
    ['liliangup']={name=liliangup,time=60*60*30},
}
...
local function on_tick()
    ...
    for l, player in pairs(game.connected_players) do
        ...
        if player.index==_ and this.qiankuang[player.index] > 0 then 
   
            local qian=30
            
            local v = player.character.get_item_count('coin')
                if v >= qian then
                    qian=30
                else
                    qian=v
            ...
            new_print(player,{'tianfu.high_debt_lose',qian,this.qiankuang[player.index]})
        end
        end
        end
...
local function liliangup(player)
    ...
    if this.mine_count[player.name]>=100 then 
        this.mine_count[player.name]=this.mine_count[player.name]-100
        local rpg_t = rpgtable.get('rpg_t')
...
local function taobing(player)
    player.character_running_speed_modifier=player.character_running_speed_modifier+1
    new_print(player,{'tianfu.taobing_over'})
...
local function ruchong(player)
    if check_tick(player,'ruchong') then 
        ...
        new_print(player,{'tianfu.ruchong_over'})
...
local function yhw(player,target)
    if check_tick(player,'yhw') then 
        ...
        new_print(player,{'tianfu.yhw_over'})
    end
end
...
local function morefali(player)
    local rpg_t = rpgtable.get('rpg_t')
    if rpg_t[player.index].mana  <= rpg_t[player.index].mana_max*0.2 then 
      if check_tick(player,'morefali') then 
       rpg_t[player.index].mana=rpg_t[player.index].mana_max
       new_print(player,{'tianfu.morefali_over'})
    end
    end
end
...
local function willdie(player)
    if check_tick(player,'willdie') then 
       player.character.health=player.character.health+200+player.character_health_bonus*0.2
...
local function on_player_used_capsule (event)
    ...
    if  have_learn(player,'morefali') and item.name=='raw-fish' then
        morefali(player)
    end
