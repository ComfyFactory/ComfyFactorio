...
local function on_entity_died(event)
    ...
    if entity_drop_amount[event.entity.name] then
        if game.forces.enemy.evolution_factor < 0.5 then
            ...
            event.entity.surface.spill_item_stack({ position = event.entity.position, stack = { name = ore_spill_raffle[math.random(1, #ore_spill_raffle)], count = amount }, enable_looted = true })
        end
    end
end
