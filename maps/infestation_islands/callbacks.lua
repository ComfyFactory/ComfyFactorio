...
Public.init_next_island_with_bridge_token =
    Scheduler.register_function(
        'init_next_island_with_bridge_token',
        function (event)
            local this = Public.get()
            local surface = event.surface
            local seed_1 = random(1, 10000000)
            ...
            Public.island_market(market, (this.current_level * random(1, 3)) * 4, false, false, true)
            ...
            if island_data.voting and island_data.voting.id then
                local suc, err = Poll.edit_poll(island_data.voting.id, 'The island ' .. this.current_level .. ' has been autogenerated.', { { text = '☺' }, { text = '☻' } }, 1)
                if not suc then
                    error(err)
                end
            end
