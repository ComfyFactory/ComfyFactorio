...
function create_beams(surface, energy, force)
    if force == 'west' then
        ...
        storage.map_forces[force].spawn = { x = -137, y = 0 }
        if energy >= 6 and energy < 12 then
            ...
            storage.map_forces[force].spawn = { x = -120, y = -32 }
        end
        if energy >= 12 and energy < 18 then
            ...
            storage.map_forces[force].spawn = { x = -120, y = -96 }
        end
        if energy >= 18 and energy < 24 then
            ...
            storage.map_forces[force].spawn = { x = -60, y = -100 }
        end
        if energy >= 24 and energy < 30 then
            ...
            storage.map_forces[force].spawn = { x = -60, y = -37 }
        end
        if energy >= 30 and energy < 36 then
            ...
            storage.map_forces[force].spawn = { x = -60, y = 28 }
        end
        if energy >= 36 and energy < 42 then
            ...
            storage.map_forces[force].spawn = { x = -60, y = 91 }
        end
        if energy >= 42 and energy < 48 then
            ...
            storage.map_forces[force].spawn = { x = 0, y = 95 }
        end
        if energy >= 48 then
            ...
            storage.map_forces[force].spawn = { x = 0, y = -33 }
        end
...
local function eat_food_from_belt(belt)
    for i = 1, 2, 1 do
        ...
        for _, science_name in pairs(Settings.science_pack_name) do
            --if storage.map_forces[belt.force.name].unit_count > storage.map_forces[belt.force.name].max_unit_count then return end
            local removed_item_count = line.remove_item({ name = science_name, count = 8 })
            storage.map_forces[belt.force.name].ate_buffer_potion[science_name] = storage.map_forces[belt.force.name].ate_buffer_potion[science_name] + removed_item_count
        end
    end
end
...
local function spawn_wave_from_belt(force_name)
    for _, science_name in pairs(Settings.science_pack_name) do
        local nb_science = storage.map_forces[force_name].ate_buffer_potion[science_name]
        if nb_science >= Settings.wave_price[science_name].price then
            ...
            storage.map_forces[force_name].ate_buffer_potion[science_name] = storage.map_forces[force_name].ate_buffer_potion[science_name] - Settings.wave_price[science_name].price
            --if storage.map_forces[belt.force.name].ate_buffer_potion[science_name] < 0 then storage.map_forces[belt.force.name].ate_buffer_potion[science_name] =0	end
        end
    end
end
...
local function send_unit_groups()
    ...
    for key, force in pairs(storage.map_forces) do
        ...
        if #units > 0 then
            ...
            local position = { x = storage.map_forces[key].spawn.x + 10 + vector[1], y = storage.map_forces[key].spawn.y + vector[2] }
...
local function randomize_worms()
    for k, pos in pairs(storage.map_forces['west'].worm_turrets_positions) do
        local vx = math.random(0, 8) - 4
        local vy = math.random(0, 8) - 4
        storage.map_forces['west'].worm_turrets_positions[k].x = pos.x + vx
        storage.map_forces['west'].worm_turrets_positions[k].y = pos.y + vy
        storage.map_forces['east'].worm_turrets_positions[k].x = storage.map_forces['east'].worm_turrets_positions[k].x - vx
        storage.map_forces['east'].worm_turrets_positions[k].y = storage.map_forces['east'].worm_turrets_positions[k].y - vy
    end
end
...
local function on_entity_died(event)
    ...
    if entity.name == 'radar' then
        storage.map_forces[entity.force.name].radar[entity.unit_number] = nil
    end
...
local function on_player_mined_entity(event)
    ...
    if event.entity.name == 'radar' then
        storage.map_forces[player.force.name].radar[event.entity.unit_number] = nil
    end
end

local function on_robot_mined_entity(event)
    if event.entity.name == 'radar' then
        storage.map_forces[event.robot.force.name].radar[event.entity.unit_number] = nil
    end
end
...
local function reveal_map(surface, force_name)
    ...
    if force_name == 'east' then
        ...
        for k, ent in pairs(storage.map_forces['east'].radar) do
    ...
    if force_name == 'west' then
        for k, ent in pairs(storage.map_forces['west'].radar) do
            if ent.energy >= 5000 then
                actif_radar_count = actif_radar_count + 1
            end
        end
...
local function tick()
    ...
    if game_tick % 900 == 0 then
        local surface = game.surfaces['native_war']
        for _, force_name in pairs({ 'west', 'east' }) do
            local structs = game.surfaces['native_war'].find_entities_filtered { position = storage.map_forces[force_name].eei, radius = 5, type = 'electric-energy-interface' }
            local energy = structs[1].energy / 100000
            if energy < storage.map_forces[force_name].energy then
                local new_global_energy = storage.map_forces[force_name].energy - 6
                if new_global_energy <= 0 then
                    new_global_energy = 0
                end
                game.print(force_name .. ' recule son beam')
                create_beams(surface, new_global_energy, force_name)
                storage.map_forces[force_name].energy = new_global_energy
            end
        end
    end
    if game_tick % 1800 == 0 then
        ...
        for _, force_name in pairs({ 'west', 'east' }) do
            local structs = game.surfaces['native_war'].find_entities_filtered { position = storage.map_forces[force_name].eei, radius = 5, type = 'electric-energy-interface' }
            local energy = structs[1].energy / 100000
            if energy >= storage.map_forces[force_name].energy + 6 then
                local new_global_energy = storage.map_forces[force_name].energy + 6
    ...
    if entity.type == 'unit' or entity.type == 'turret' then
        if cause.type == 'unit' then
            if cause.name == 'small-biter' or cause.name == 'medium-biter' or cause.name == 'big-biter' or cause.name == 'behemoth-biter' then
                local modified_damage = event.original_damage_amount * storage.map_forces[cause.force.name].modifier.damage * storage.map_forces[entity.force.name].modifier.resistance
            ...
            elseif cause.name == 'small-spitter' or cause.name == 'medium-spitter' or cause.name == 'big-spitter' or cause.name == 'behemoth-spitter' then
                local modified_damage = event.original_damage_amount * storage.map_forces[cause.force.name].modifier.splash * storage.map_forces[entity.force.name].modifier.resistance
                entity.health = entity.health - modified_damage
            end
        end
    end
...
local function show_color_force()
    ...
    for _, unit in pairs(storage.map_forces[force_name1].units) do
    ...
    for _, unit in pairs(storage.map_forces[force_name2].units) do
        rendering.draw_circle {
            color = { 1, 1, 255 },
            radius = 0.1,
            filled = true,
