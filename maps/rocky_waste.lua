...
storage.rock_yield_amount_modifier = 0.5
...
local function init_surface()
    if game.surfaces['rocky_waste'] then
        return game.surfaces['rocky_waste']
    end
...
local function on_player_mined_entity(event)
    ...
    if math.random(1, 256) == 1 then
        unearthing_biters(entity.surface, entity.position, math.random(4, 16))
        return
    end
end
