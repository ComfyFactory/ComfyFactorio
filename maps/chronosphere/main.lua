...
local function generate_overworld(surface, optplanet)
    ...
    local message = {
        'chronosphere.planet_jump',
        planet[1].name.name,
        planet[1].ore_richness.name,
        planet[1].day_speed.name
    }
    game.print(message, {r = 0.98, g = 0.66, b = 0.22})
    local discordmessage =
        'Destination: ' ..
        planet[1].name.dname ..
            ', Ore Richness: ' .. planet[1].ore_richness.dname .. ', Daynight cycle: ' .. planet[1].day_speed.dname
...
local function reset_map()
    ...
    if game.surfaces['chronosphere'] then
        game.delete_surface(game.surfaces['chronosphere'])
    end
    ...
    if not objective.active_surface_index then
        ...
        objective.active_surface_index =
            Reset.soft_reset_map(
            game.surfaces[objective.active_surface_index],
            Chrono.get_map_gen_settings(),
            starting_items
        ).index
    end
...
local function on_player_joined_game(event)
    ...
    if not objective.flame_boots[event.player_index].steps then
        objective.flame_boots[event.player_index].steps = {}
    end
...
function Public.chronojump(choice)
    ...
    objective.lab_cells = {}
    objective.active_surface_index =
        game.create_surface('chronosphere' .. objective.chronojumps, Chrono.get_map_gen_settings()).index
    local surface = game.surfaces[objective.active_surface_index]
    ...
    surface.pollute(
        ...
        150 * (3 / (objective.upgrades[2] / 3 + 1)) * (1 + objective.chronojumps) * Diff.difficulty_vote_value
    )
    ::continue::
end
                ...
                if objective.locomotive ~= nil then
                    ...
                    if surface and surface.valid then
                        game.surfaces[objective.active_surface_index].pollute(
                            pos,
                            (0.5 * objective.chronojumps) * (3 / (objective.upgrades[2] / 3 + 1)) *
...
local function on_player_mined_entity(event)
    ...
    if entity.name == 'rock-huge' or entity.name == 'rock-big' or entity.name == 'sand-rock-big' then
        ...
        elseif objective.planet[1].name.id == 11 then
            event.buffer.clear() -- rocky planet
        end
    end
end
