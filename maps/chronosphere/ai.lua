...
Public.destroy_inactive_biters = function()
    local objective = Chrono_table.get_table()
    if objective.chronotimer < 100 then
        return
    end
...
Public.send_near_biters_to_objective = function()
    ...
    local success = false
    if pollution > 200 * (1 / Diff.difficulty_vote_value) or objective.planet[1].name.id == 17 then
        surface.pollute(random_target.position, -50 * (1 / Diff.difficulty_vote_value))
        --game.print("sending objective wave")
        success = true
    else
        if objective.chronojumps < 50 then
            --game.print("not enough pollution for objective attack")
            if math_random(1, 50 - objective.chronojumps) == 1 then
                success = true
            end
        else
            success = true
        ...
        game.surfaces[objective.active_surface_index].set_multi_command(
            {
                ...
                unit_count = 16 +
                    math_random(1, math_floor(1 + game.forces['enemy'].evolution_factor * 100)) *
                        Diff.difficulty_vote_value,
                force = 'enemy',
...
local function get_random_close_spawner(surface)
    ...
    for i = 1, 5, 1 do
        ...
        if spawner_2.position.x ^ 2 + spawner_2.position.y ^ 2 < spawner.position.x ^ 2 + spawner.position.y ^ 2 then
            spawner = spawner_2
        end
    end
...
local function select_units_around_spawner(spawner)
    ...
    local unit_count = 0
    local max_unit_count = 128 * Diff.difficulty_vote_value
...
local function send_group(unit_group, nearest_player_unit)
    ...
    if pollution > 200 * (1 / Diff.difficulty_vote_value) or objective.planet[1].name.id == 17 then
        surface.pollute(target.position, -50 * (1 / Diff.difficulty_vote_value))
...
local function is_chunk_empty(surface, area)
    if surface.count_entities_filtered({type = {'unit-spawner', 'unit'}, area = area}) ~= 0 then
        return false
    end
    if surface.count_entities_filtered({force = 'player', area = area}) ~= 0 then
        return false
    end
...
local function get_unit_group_position(surface, nearest_player_unit, spawner)
    if math_random(1, 3) ~= 1 then
        local spawner_chunk_position = {
            x = math.floor(spawner.position.x / 32),
            y = math.floor(spawner.position.y / 32)
        }
        local valid_chunks = {}
        for x = -2, 2, 1 do
            for y = -2, 2, 1 do
    ...
    local unit_group_position = {
        x = (spawner.position.x + nearest_player_unit.position.x * 0.2),
        y = (spawner.position.y + nearest_player_unit.position.y * 0.2)
    }
...
local function create_attack_group(surface)
    ...
    local nearest_player_unit =
        surface.find_nearest_enemy({position = spawner.position, max_distance = 1024, force = 'enemy'})
    if not nearest_player_unit then
        nearest_player_unit = objective.locomotive
    end
                ...
                if unit_group then
                    if unit_group.valid then
                        if unit_group.state == defines.group_state.finished then
                            local nearest_player_unit =
                                entity.surface.find_nearest_enemy(
                                {position = entity.position, max_distance = 2048, force = 'enemy'}
                            )
                            if not nearest_player_unit then
                                nearest_player_unit = objective.locomotive
                            end
                            local destination =
                                unit_group.surface.find_non_colliding_position(
                                'rocket-silo',
                                unit_group.position,
                                32,
                                1
                            ...
                            if not destination then
                                destination = {
                                    x = unit_group.position.x + math_random(-10, 10),
                                    y = unit_group.position.y + math_random(-10, 10)
                                }
                            end
                            unit_group.set_command(
                                {
                            ...
                            return
                        elseif
                            unit_group.state == defines.group_state.gathering and
                                not unit_group.surface.find_non_colliding_position(
                                    'rocket-silo',
                                    unit_group.position,
                                    3,
                                    1
