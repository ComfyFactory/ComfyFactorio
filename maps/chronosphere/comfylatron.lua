...
local function is_target_inside_habitat(pos, surface)
    ...
    if pos.x < objective.comfylatron_habitat.left_top.x then
        return false
    end
    if pos.x > objective.comfylatron_habitat.right_bottom.x then
        return false
    end
    if pos.y < objective.comfylatron_habitat.left_top.y then
        return false
    end
    if pos.y > objective.comfylatron_habitat.right_bottom.y then
        return false
    end
    return true
end
...
local function visit_player()
    local objective = Chrono_table.get_table()
    if objective.comfylatron_last_player_visit > game.tick then
        return false
    end
...
local function spawn_comfylatron(surface_index, x, y)
    ...
    if objective.comfylatron_disabled then
        return false
    end
    if not objective.comfylatron_last_player_visit then
        objective.comfylatron_last_player_visit = 0
    end
...
local function heartbeat()
    ...
    if not objective.comfylatron then
        if math_random(1, 4) == 1 then
            spawn_comfylatron(game.surfaces['cargo_wagon'].index, 0, -128)
        end
        return
    end
    if not objective.comfylatron.valid then
        objective.comfylatron = nil
        return
    end
...
local function on_entity_damaged(event)
    ...
    if event.entity ~= objective.comfylatron then
        return
    end
    desync(event)
end
